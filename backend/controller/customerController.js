...
const loginCustomer = async (req, res) => {
  try {
    ...
    if (customer && customer.password && bcrypt.compareSync(req.body.password, customer.password)) {
...
const changePassword = async (req, res) => {
  try {
    ...
    } else if (customer && bcrypt.compareSync(req.body.currentPassword, customer.password)) {
      customer.password = bcrypt.hashSync(req.body.newPassword);
      await customer.save();
      res.send({
...
const findCustomersWithOrders = async (req, res) => {
  try {
    // Fetch all orders and select only user_info.email
    const orders = await Order.find({}).select("user_info.email -_id");
    // Extract unique email addresses
    const emails = [...new Set(orders.map((order) => order.user_info.email))];
    // Find customers by these emails
