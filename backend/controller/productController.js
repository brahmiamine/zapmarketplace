...
const getShowingStoreProducts = async (req, res) => {
  ...
  try {
    ...
    if (slug) {
      products = await Product.find(queryObject).populate({ path: "category", select: "name _id" }).sort({ _id: -1 }).limit(100);
      ...
      popularProducts = await Product.find(queryObject).populate({ path: "category", select: "name _id" }).sort({ sales: -1 }).limit(20);
