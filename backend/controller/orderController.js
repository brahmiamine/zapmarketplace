...
const getAllOrders = async (req, res) => {
  ...
  if (customerName) {
    queryObject.$or = [{ "user_info.name": { $regex: `${customerName}`, $options: "i" } }, { invoice: { $regex: `${customerName}`, $options: "i" } }];
  }
  ...
  try {
    ...
    const orders = await Order.find(queryObject)
      .select("_id invoice paymentMethod subTotal total user_info discount shippingCost status createdAt updatedAt code")
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(limits);
    let methodTotals = [];
    if (startDate && endDate) {
      ...
      for (const order of filteredOrders) {
        ...
        const existPayment = methodTotals.find((item) => item.method === paymentMethod);
...
const getOrderStaff = async (req, res) => {
  console.log(req.params);
  try {
    const orders = await Order.find({ code: req.params.code }).sort({ _id: -1 });
    res.send(orders);
  } catch (err) {
    res.status(500).send({
      message: err.message,
...
const getDashboardRecentOrder = async (req, res) => {
  try {
    ...
    if (code) {
      queryObject.code = code; // Assuming the 'code' field exists in your Order model
    }

    const totalDoc = await Order.countDocuments(queryObject);

    // query for orders
    const orders = await Order.find(queryObject).sort({ updatedAt: -1 }).skip(skip).limit(limits);
...
const getDashboardCount = async (req, res) => {
  try {
    ...
    const matchQuery = code ? { code } : {}; // Ad
    const totalDoc = await Order.countDocuments(matchQuery);
    ...
    const totalProcessingOrder = await Order.aggregate([
      {
        $match: { ...matchQuery, status: "Processing" },
      },
      {
        $group: {
          _id: null,
...
const getDashboardOrders = async (req, res) => {
  ...
  try {
    ...
    // query for orders
    const orders = await Order.find({}).sort({ _id: -1 }).skip(skip).limit(limits);
