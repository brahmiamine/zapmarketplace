...
const Uploader = ({ setImageUrl, imageUrl, product, folder }) => {
  ...
  useEffect(() => {
    ...
    if (files) {
      files.forEach((file) => {
        ...
        if (product) {
          const result = imageUrl?.find((img) => img === `${import.meta.env.VITE_APP_CLOUDINARY_URL}`);
        ...
        axios({
          url: `${import.meta.env.VITE_APP_API_BASE_URL}/file`,
          method: "POST",
          headers: {
  ...
  return (
    <div className="w-full text-center">
      <div className="border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md cursor-pointer px-6 pt-5 pb-6" {...getRootProps()}>
        <input {...getInputProps()} />
        ...
        {product ? (
          ...
          <div className="relative">
            ...
            <img
              ...
              alt="product"
            />
            <button type="button" className="absolute top-0 right-0 text-red-500 focus:outline-none" onClick={() => handleRemoveImage(imageUrl)}>
              <FiXCircle />
            </button>
          </div>
        ) : (
