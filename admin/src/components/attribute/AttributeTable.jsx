...
const AttributeTable = ({ isCheck, setIsCheck, attributes }) => {
  ...
  const filteredArray = filterArrayByCode(attributes, adminInfo?.role, adminInfo?.code);
      ...
      <TableBody>
        {filteredArray?.map((attribute) => (
          <TableRow key={attribute._id}>
            <TableCell>
              <CheckBox type="checkbox" name="attribute" id={attribute._id} handleClick={handleClick} isChecked={isCheck?.includes(attribute._id)} />
            </TableCell>
            ...
            <TableCell className="font-medium text-sm">{attribute?.code}</TableCell>
            ...
            <TableCell className="flex justify-center">
              <Link to={`/attributes/${attribute._id}`} className="p-2 cursor-pointer text-gray-400 hover:text-emerald-600 focus:outline-none">
                <Tooltip id="edit values" Icon={FiEdit} title="Edit Values" bgColor="#10B981" />
              </Link>
            </TableCell>
