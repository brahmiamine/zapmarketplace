...
const CouponTable = ({ isCheck, coupons, setIsCheck }) => {
  ...
  const filteredArray = filterArrayByCode(coupons, adminInfo?.role, adminInfo?.code);
          ...
          <TableRow key={i + 1}>
            ...
            <TableCell>
              <div className="flex items-center">
                {coupon?.logo ? (
                  <Avatar className="hidden p-1 mr-2 md:block bg-gray-50 shadow-none" src={getValidImagePath(coupon?.logo)} alt="product" />
                ) : (
            ...
            {coupon?.discountType?.type ? (
              <TableCell>
                ...
                <span className="text-sm font-semibold">
                  {" "}
                  {coupon?.discountType?.type === "percentage" ? `${coupon?.discountType?.value}%` : `${currency}${coupon?.discountType?.value}`}
                </span>{" "}
              </TableCell>
            ) : (
              <TableCell>
