...
const useProductSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (data.originalPrice < data.price) {
        ...
        return notifyError("Sale Price must be less then or equal of product price!");
      }
      ...
      if (adminInfo.code && adminInfo.role !== "Admin") {
        productData.code = adminInfo.code;
      }
      ...
      if (updatedId) {
        ...
        if (isCombination) {
          ...
          setSku(res.sku);
          const result = res.variants.map(({ originalPrice, price, discount, quantity, barcode, sku, productId, image, ...rest }) => rest);
  ...
  const handleGenerateCombination = () => {
    ...
    const result = variants.filter(
      ({ originalPrice, discount, price, quantity, barcode, sku, productId, image, ...rest }) => JSON.stringify({ ...rest }) !== "{}"
    );
  ...
  const handleProductTap = (e, value, name) => {
    ...
    if (value) {
      ...
      if (!isBasicComplete) return notifyError(`${"Please save product before adding combinations!"}`);
    }
    setTapValue(e);
  };
