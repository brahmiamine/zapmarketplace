...
const DeliverySettings = () => {
  ...
  const { data, loading, error } = useAsync(DeliveryServices.getAllDelivery);
  ...
  const handleFilter = () => {
    const searchTerm = couponRef.current.value.toLowerCase();
    const newFilteredData = data.filter((item) => item.name.toLowerCase().includes(searchTerm));
    setFilteredData(newFilteredData);
  };

  return (
    <>
      <PageTitle>{t("Delivery")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title="Selected Delivery" />
      <BulkActionDrawer ids={allId} title="Delivery" />
      <MainDrawer>
          ...
          <form onSubmit={handleSubmitCoupon} className="py-3 grid gap-4 lg:gap-6 xl:gap-6  xl:flex">
            ...
            <div className="lg:flex  md:flex xl:justify-end xl:w-1/2  md:w-full md:justify-start flex-grow-0">
              ...
              <div className="w-full md:w-32 lg:w-32 xl:w-32 mr-3 mb-3 lg:mb-0">
                <Button disabled={isCheck.length < 1} onClick={() => handleDeleteMany(isCheck)} className="w-full rounded-md h-12 bg-red-500 btn-red">
                  <span className="mr-2">
                    <FiTrash2 />
                  </span>
        ...
        <CardBody>
          <form onSubmit={handleSubmitCoupon} className="py-3 grid gap-4 lg:gap-6 xl:gap-6 md:flex xl:flex">
            <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
              <Input ref={couponRef} type="search" placeholder={t("Search by delivery")} />
            </div>
            <div className="flex items-center gap-2 flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
              <div className="w-full mx-1">
                <Button onClick={handleFilter} className="h-12 w-full bg-emerald-700">
                  Filter
                </Button>
              </div>
      ...
      {loading ? (
        ...
        <TableContainer className="mb-8">
          <Table>
            ...
            <DeliveryTable lang={lang} isCheck={isCheck} data={filteredData} setIsCheck={setIsCheck} />
          </Table>
          <TableFooter>
