        ...
        <div className="inline-block overflow-y-auto h-full align-middle transition-all transform">
          <div className="flex flex-col lg:flex-row md:flex-row w-full overflow-hidden">
            <div className="flex-shrink-0 flex items-center justify-center h-auto">
              {data?.image[0] ? (
                <img src={getValidImagePath(data?.image[0])} alt="product" className="h-64 w-64" />
              ) : (
                <img src="https://res.cloudinary.com/dapzqelui/image/upload/v1702715308/images/placeholder_kvepfp.png" alt="product" />
              )}
            </div>
            <div className="w-full flex flex-col p-5 md:p-8 text-left">
              <div className="mb-5 block ">
                <div className="font-serif font-semibold py-1 text-sm">
                  <p className="text-sm text-gray-500 pr-4">
                    {t("Status")}:{" "}
                    {data.status === "show" ? (
                      <span className="text-emerald-400">{t("ThisProductShowing")}</span>
                    ) : (
              ...
              <p className="text-sm leading-6 text-gray-500 dark:text-gray-400 md:leading-7">{showingTranslateValue(data?.description)}</p>
              <div className="flex flex-col mt-4">
                <p className="font-serif font-semibold py-1 text-gray-500 text-sm">
                  <span className="text-gray-700 dark:text-gray-400">{t("Category")}: </span> {showingTranslateValue(data?.category?.name)}
                </p>
      ...
      {data?.isCombination && variantTitle?.length > 0 && !loading && (
        <>
          ...
          <TableContainer className="mb-8 rounded-b-lg">
            ...
            <TableFooter>
              <Pagination totalResults={totalResults} resultsPerPage={resultsPerPage} onChange={handleChangePage} label="Product Page Navigation" />
            </TableFooter>
          </TableContainer>
        </>
      )}
