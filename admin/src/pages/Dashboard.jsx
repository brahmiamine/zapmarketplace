import { Pagination, Table, TableCell, TableContainer, TableFooter, TableHeader, WindmillContext } from "@windmill/react-ui";
...
const Dashboard = () => {
  ...
  const { data: dashboardRecentOrder, loading: loadingRecentOrder } = useAsync(() =>
    OrderServices.getDashboardRecentOrder({ page: currentPage, limit: 8, code: adminInfo?.code })
  );
  const { data: dashboardOrderCount, loading: loadingOrderCount } = useAsync(() => OrderServices.getDashboardCount(adminInfo?.code));
  const { data: dashboardOrderAmount, loading: loadingOrderAmount } = useAsync(() => OrderServices.getDashboardAmount(adminInfo?.code));
  ...
  useEffect(() => {
    ...
    // yesterday orders
    const yesterdayOrder = dashboardOrderAmount?.ordersData?.filter((order) => dayjs(order.updatedAt).set(-1, "day").isYesterday());
  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-2 my-8">
        <ChartCard mode={mode} loading={loadingOrderAmount} title={t("WeeklySales")}>
          <LineChart salesReport={salesReport} />
        </ChartCard>

        <ChartCard mode={mode} loading={loadingBestSellerProduct} title={t("BestSellingProducts")}>
      ...
      {loadingRecentOrder ? (
        ...
        <TableContainer className="mb-8">
          ...
          <TableFooter>
            <Pagination totalResults={dashboardRecentOrder?.totalOrder} resultsPerPage={8} onChange={handleChangePage} label="Table navigation" />
          </TableFooter>
        </TableContainer>
      ) : (
