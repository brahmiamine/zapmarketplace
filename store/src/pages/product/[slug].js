...
const ProductScreen = ({ product, attributes, relatedProducts }) => {
  ...
  useEffect(() => {
    if (value) {
      ...
      const res = result?.map(({ originalPrice, discount, quantity, inUse, inUseOrder, barcode, sku, productId, image, ...rest }) => ({ ...rest }));
      // console.log("res", res);
  ...
  const handleAddToCart = (p) => {
    if (p.variants.length === 1 && p.variants[0].quantity < 1) return notifyError("Insufficient stock");
            ...
            <div className="mx-auto px-3 lg:px-10 max-w-screen-2xl">
              <div className="flex items-center pb-4">
                <ol className="flex items-center w-full overflow-hidden font-serif">
                  ...
                  <li className="text-sm px-1 transition duration-200 ease-in ">{showingTranslateValue(product?.title)}</li>
                </ol>
              </div>
              <div className="w-full rounded-lg p-3 lg:p-12 bg-white">
                <div className="flex flex-col xl:flex-row">
                  <div className="flex-shrink-0 xl:pr-10 lg:block w-full mx-auto md:w-6/12 lg:w-5/12 xl:w-4/12">
                    ...
                    {product.image[0] ? (
                      <Image src={getValidImagePath(img || product.image[0])} alt="product" width={650} height={650} priority />
                    ) : (
                      <Image
                        src="https://res.cloudinary.com/dapzqelui/image/upload/v1702715308/images/placeholder_kvepfp.png"
                        width={650}
                        height={650}
                        alt="product Image"
                      />
                      ...
                      <div className=" w-3/5 xl:pr-6 md:pr-6  md:w-2/3 mob-w-full">
                        ...
                        <div>
                          <div className="text-sm leading-6 text-gray-500 md:leading-7">
                            {isReadMore ? showingTranslateValue(product?.description)?.slice(0, 230) : showingTranslateValue(product?.description)}
                            <br />
                          ...
                          <div className="mt-8">
                            <p className="text-xs sm:text-sm text-gray-700 font-medium">
                              {t("common:Call Us To Order By Mobile Number")} : <span className="text-emerald-700 font-semibold">+0044235234</span>{" "}
                            </p>
                          </div>

                          {/* social share */}
                          <div className="mt-2">
                            <h3 className="text-base font-semibold mb-1 font-serif">{t("common:shareYourSocial")}</h3>
                            <p className="font-sans text-sm text-gray-500">{t("common:shareYourSocialText")}</p>
                            <ul className="flex mt-4">
                              <li className="flex items-center text-center border border-gray-100 rounded-full hover:bg-emerald-500  mr-2 transition ease-in-out duration-500">
                                <FacebookShareButton url={`https://store.zapmarketplace.online/product/${router.query.slug}`} quote="">
                                  <FacebookIcon size={32} round />
                                </FacebookShareButton>
                              </li>
                              <li className="flex items-center text-center border border-gray-100 rounded-full hover:bg-emerald-500  mr-2 transition ease-in-out duration-500">
                                <TwitterShareButton url={`https://store.zapmarketplace.online/product/${router.query.slug}`} quote="">
                                  <TwitterIcon size={32} round />
                                </TwitterShareButton>
                              </li>
                              <li className="flex items-center text-center border border-gray-100 rounded-full hover:bg-emerald-500  mr-2 transition ease-in-out duration-500">
                                <RedditShareButton url={`https://store.zapmarketplace.online/product/${router.query.slug}`} quote="">
                                  <RedditIcon size={32} round />
                                </RedditShareButton>
                              </li>
                              <li className="flex items-center text-center border border-gray-100 rounded-full hover:bg-emerald-500  mr-2 transition ease-in-out duration-500">
                                <WhatsappShareButton url={`https://store.zapmarketplace.online/product/${router.query.slug}`} quote="">
                                  <WhatsappIcon size={32} round />
                                </WhatsappShareButton>
                              </li>
                              <li className="flex items-center text-center border border-gray-100 rounded-full hover:bg-emerald-500  mr-2 transition ease-in-out duration-500">
                                <LinkedinShareButton url={`https://store.zapmarketplace.online/product/${router.query.slug}`} quote="">
              ...
              {relatedProducts?.length >= 2 && (
                <div className="pt-10 lg:pt-20 lg:pb-10">
                  <h3 className="leading-7 text-lg lg:text-xl mb-3 font-semibold font-serif hover:text-gray-600">{t("common:relatedProducts")}</h3>
                  <div className="flex">
                    <div className="w-full">
