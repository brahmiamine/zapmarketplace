  ...
  useEffect(() => {
    ...
    if (!loading && !error && storeSetting?.google_analytic_status) {
      ...
      const handleRouteChange = (url) => {
        handlePageView(`/${router.pathname}`, "ZAPMarketplace");
      };
  ...
  return (
    <>
      {!loading && !error && storeSetting?.tawk_chat_status && (
        <TawkMessengerReact propertyId={storeSetting?.tawk_chat_property_id || ""} widgetId={storeSetting?.tawk_chat_widget_id || ""} />
      )}
