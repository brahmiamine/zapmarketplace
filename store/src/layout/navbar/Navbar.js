...
const Navbar = () => {
  ...
  useEffect(() => {
    const color = storeCustomizationSetting?.navbar?.nav_color;
    document.documentElement.style.setProperty("--primary-color", color || "#FFFFFF");
          ...
          <div className="top-bar h-16 lg:h-auto flex items-center justify-between py-4 mx-auto">
            <Link href="/" className="mr-3 lg:mr-12 xl:mr-12 hidden md:hidden lg:block">
              <Image
                width={130}
                height={43}
                src={getValidImagePath(storeCustomizationSetting?.navbar?.logo || "/logo/logo-light.svg")}
                alt="logo"
            ...
            <div className="w-full transition-all duration-200 ease-in-out lg:flex lg:max-w-[520px] xl:max-w-[750px] 2xl:max-w-[900px] md:mx-12 lg:mx-4 xl:mx-0">
              <div className="w-full flex flex-col justify-center flex-shrink-0 relative z-30">
                <div className="flex flex-col mx-auto w-full">
                  <form onSubmit={handleSubmit} className="relative pr-12 md:pr-14 bg-white overflow-hidden shadow-sm rounded-md w-full">
            ...
            <div className="hidden md:hidden md:items-center lg:flex xl:block absolute inset-y-0 right-0 pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
              ...
              <button aria-label="Total" onClick={toggleCartDrawer} className="relative px-5 text-white text-2xl font-bold">
              ...
              <button className="pl-5 text-white text-2xl font-bold" aria-label="Login">
                {imageUrl || userInfo?.image ? (
                  <Link href="/user/dashboard" className="relative top-1 w-6 h-6">
                    <Image width={29} height={29} src={getValidImagePath(imageUrl || userInfo?.image)} alt="user" className="bg-white rounded-full" />
                  </Link>
                ) : userInfo?.name ? (
