...
const ProductCard = ({ product, attributes }) => {
  ...
  useEffect(() => {
    setMin(product.min || 1);
  }, [product]);

  const handleAddItem = (p) => {
    if (p.stock < 1) return notifyError("Insufficient stock!");
    const minQuantity = p.min || 1; // Use product.min if available, otherwise default to 1
    setMin(p.min || 1);
    ...
    const newItem = {
      ...
      slug: p.slug,
      quantity: minQuantity, // Set initial quantity to minQuantity
    };
    console.log(newItem);
    addItem(newItem);
    console.log(items);
  ...
  return (
    <>
      {modalOpen && <ProductModal modalOpen={modalOpen} setModalOpen={setModalOpen} product={product} currency={currency} attributes={attributes} />}
      ...
      <div className="group box-border overflow-hidden flex rounded-md shadow-sm pe-0 flex-col items-center bg-white relative">
        ...
        >
          <div className="relative w-full h-full p-2">
            {product.image[0] ? (
              <ImageWithFallback src={getValidImagePath(product.image[0])} alt="product" />
            ) : (
              <Image
                ...
                {items.map(
                  (item) =>
                    item.id === product._id && (
                      ...
                      >
                        {/* disabled={product.min >= item.quantity ? true : false} */}
                        <button onClick={() => updateItemQuantity(item.id, item.quantity - 1)}>
                          <span className="text-dark text-base">
                            <IoRemove />
                          </span>
                        </button>
                        {min && <p className="text-sm text-dark px-1 font-serif font-semibold">{item.quantity}</p>}
