...
const Cart = () => {
  ...
  const checkoutClass = (
    ...
    >
      <span className="align-middle font-medium font-serif">{t("common:proceedToCheckoutBtn")}</span>
    ...
    <>
      ...
      <div className="flex flex-col w-full h-full justify-between items-middle bg-white rounded cursor-pointer">
        <div className="w-full flex justify-between items-center relative px-5 py-4 border-b bg-indigo-50 border-gray-100">
          ...
          >
            <IoClose />
            <span className="font-sens text-sm text-gray-500 hover:text-red-400 ml-1">{t("common:closeBtn")}</span>
          </button>
        </div>
          ...
          {isEmpty && (
            <div className="flex flex-col h-full justify-center">
              <div className="flex flex-col items-center">
                ...
                <h3 className="font-serif font-semibold text-gray-700 text-lg pt-5">Your cart is empty</h3>
                <p className="px-12 text-center text-sm text-gray-500 pt-2">No items added in your cart. Please add product to your cart list.</p>
              </div>
            </div>
          )}
        ...
        <div className="mx-5 my-3">
          {items.length <= 0 ? (
            ...
            <span>{!userInfo ? <div onClick={handleOpenLogin}>{checkoutClass}</div> : <Link href="/checkout">{checkoutClass}</Link>}</span>
          )}
        </div>
      </div>
    </>
