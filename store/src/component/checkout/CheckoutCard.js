  ...
  return (
    ...
    >
      <div className="relative flex rounded-md overflow-hidden flex-shrink-0 cursor-pointer mr-4">
        <Image src={getValidImagePath(item.image)} width={50} height={50} alt={item.title} />
      </div>
      <div className="flex flex-col w-full overflow-hidden">
        <div className="flex items-center justify-between">
          <p className="mb-0">
            <span className="text-sm font-medium text-gray-700 text-heading line-clamp-1">{item?.title?.substring(0, 5)}</span>
